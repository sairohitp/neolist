...
export function useLocalStorage<T>(key: string, initialValue: T): [T, (value: T | ((val: T) => T)) => void] {
  ...
  useEffect(() => {
    // This effect is to sync the state if the localStorage is changed by another tab.
    ...
    // Initial sync in case the value was set before this hook mounted in this render.
    setStoredValue(getValueFromLocalStorage(key, initialValue));
